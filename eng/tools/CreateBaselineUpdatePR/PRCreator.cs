...
public class PRCreator
{
    ...
    private List<string> GetUpdatedFiles(string updatedFilesDirectory) =>
        Directory.GetFiles(updatedFilesDirectory, "Updated*", SearchOption.AllDirectories).ToList();

    private async Task<List<NewTreeItem>> UpdateAllFilesAsync(List<string> updatedFiles, List<NewTreeItem> tree, Pipelines pipeline)
    {
        ...
        foreach (var filePath in updatedFiles)
        {
            ...
            if (defaultContent != null && content == defaultContent)
            {
                content = null;
            }
